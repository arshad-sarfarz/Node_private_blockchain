 ...
 ******************************************/
//This function need to be commented when validation methods are executed
...
myBlockChain.getBlockHeight().then((height) => {
	console.log("Height of the block is " + height);
...
myBlockChain.getBlock(20).then((block) => {
	console.log(JSON.stringify(block));
}).catch((err) => { console.log(err);});
...
myBlockChain.validateBlock(15).then((valid) => {
	console.log("valid="+ valid);
})
.catch((error) => {
	console.log(error);
